...
export function useFormValidation() {
  ...
  function validateField(fieldName, value, rules = []) {
    for (const rule of rules) {
      const ruleFn = validationRules[rule.rule]
      if (ruleFn) {
...
function validateField(fieldName, value, rules) {
  const validationRules = {
    required: (v, rule) => !v && v !== 0 && v !== false ? (rule.message || '此字段必填') : null,
    email: (v) => v && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v) ? '请输入有效的邮箱地址' : null,
    phone: (v) => v && !/^1[3-9]\d{9}$/.test(v.replace(/\s/g, '')) ? '请输入有效的手机号码' : null,
    url: (v) => v && !/^https?:\/\/[^\s]+$/.test(v) ? '请输入有效的网址' : null,
    min: (v, rule) => {
      const min = parseFloat(rule.value)
      return (typeof v === 'string' && v.length < min) || (typeof v === 'number' && v < min) 
        ? (rule.message || `至少需要${min}个字符`) : null
    },
    max: (v, rule) => {
      const max = parseFloat(rule.value)
      return (typeof v === 'string' && v.length > max) || (typeof v === 'number' && v > max) 
        ? (rule.message || `最多${max}个字符`) : null
    },
    pattern: (v, rule) => v && rule.value && !new RegExp(rule.value).test(v) 
      ? (rule.message || '格式不正确') : null,
    numeric: (v) => v && isNaN(parseFloat(v)) ? '请输入数字' : null,
    positive: (v) => !isNaN(parseFloat(v)) && parseFloat(v) <= 0 ? '请输入正数' : null,
    decimal: (v, rule) => {
      const decimals = rule.value || 2
      return v && !new RegExp(`^\\d+(\\.\\d{1,${decimals}})?$`).test(v) 
        ? (rule.message || `最多保留${decimals}位小数`) : null
    },
  }

  for (const rule of rules) {
    const fn = validationRules[rule.rule]
    if (fn) {
      const err = fn(value, rule)
      if (err) return err
    }
  }
  return null
