<template>
  ...
  <BtModal v-model:visible="showServiceItemForm" title="添加服务明细" icon="bi bi-clock-history" max-width="500px">
    <template #body>
      ...
      <div class="mb-3">
        ...
        <select v-model="serviceItemForm.service_fee_id" class="form-select">
          ...
          <option v-for="sf in availableServiceFees" :key="sf.id" :value="sf.id">
            {{ sf.name }} (¥{{ sf.base_price || sf.unit_price || 0 }}/h)
          </option>
        </select>
      </div>
      <div class="mb-3">
      ...
      <div v-if="serviceItemForm.technician_id && serviceItemForm.service_fee_id && serviceItemForm.hours" class="mb-3 p-3 rounded-lg" style="background:var(--bt-gray-50, #f8f9fa)">
    ...
    <template #footer>
      ...
      <button class="btn btn-primary btn-sm" @click="addServiceItem"
              :disabled="!serviceItemForm.technician_id || !serviceItemForm.service_fee_id || !serviceItemForm.hours">
        确认添加
      </button>
    </template>
  </BtModal>
...
const serviceItemForm = ref({ technician_id: null, service_fee_id: null, hours: 1 })
...
const serviceItemCost = computed(() => (currentTechnician.value?.cost_rate || 0) * (serviceItemForm.value.hours || 0))
const serviceItemRevenue = computed(() => (currentServiceFee.value?.base_price || currentServiceFee.value?.unit_price || 0) * (serviceItemForm.value.hours || 0))
...
function openServiceItemForm() {
  serviceItemForm.value = { technician_id: null, service_fee_id: null, hours: 1 }
  showServiceItemForm.value = true
}

async function addServiceItem() {
  if (!serviceItemForm.value.technician_id || !serviceItemForm.value.service_fee_id || !serviceItemForm.value.hours) return
  ...
  try {
    await ticketApi.addServiceItem(props.id, {
      ...
      name: sf?.name || '',
      hours: serviceItemForm.value.hours,
      unit_price: sf?.base_price || sf?.unit_price || 0,
      cost_price: tech?.cost_rate || 0,
    })
