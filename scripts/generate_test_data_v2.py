...
def generate_test_data():
    ...
    # goods 表: name, category_id, selling_price, cost_price, min_stock, unit, ...
    goods_data = [
    ...
    print("3️⃣  创建供应商数据 ...")
    # suppliers 表: name, contact, phone, payment_terms, ...
    suppliers = [
    ...
    print("4️⃣  创建技术员数据 ...")
    # technicians 表: name, phone, skills, cost_rate, billing_type, daily_cost_rate, package_cost, ...
    technicians = [
        ("苏鹏", "13811112222", "网络/系统/硬件", "hourly", 50, 0, 0, 60),
        ("王浩博", "13822223333", "系统安装/维护", "hourly", 35, 0, 0, 45),
        ("李学平", "13833334444", "网络/监控", "hourly", 40, 0, 0, 50),
        ("小赵", "13844445555", "基础维修", "hourly", 25, 0, 0, 30),
    ]
    tech_ids = {}
    for name, phone, skills, billing_type, cost_rate, daily_rate, package_rate, labor_cost in technicians:
        existing = db_query_one("SELECT id FROM technicians WHERE name = ?", (name,))
        if existing:
            tech_ids[name] = existing['id']
            db_execute("""UPDATE technicians SET phone=?, skills=?, cost_rate=?, billing_type=?,
                          labor_cost=?, labor_revenue=? WHERE id=?""",
                       (phone, skills, cost_rate, billing_type, labor_cost, 0, existing['id']))
        else:
            tid = db_execute("""INSERT INTO technicians (name, phone, skills, cost_rate, billing_type, 
                               daily_cost_rate, package_cost, labor_cost, created_at)
                               VALUES (?,?,?,?,?,?,?,?,?)""",
                           (name, phone, skills, cost_rate, billing_type, daily_rate, package_rate, labor_cost, today_ts))
            tech_ids[name] = tid
    ...
    for client, title, status, total, priority in ticket_plans:
        ...
        if existing:
            ...
            tid = db_execute("INSERT INTO tickets (client, title, status, total, priority, created_at) VALUES (?,?,?,?,?,?)",
    ...
    for goods_name, qty in [
        ("TP-LINK千兆路由器", 5), ("超五类网线305米箱", 3), ("海康威视摄像头", 8),
        ("电脑电源500W", 10), ("22寸显示器", 4), ("HDMI线2米", 2),
        ("VGA线1.5米", 1), ("联想电源适配器", 10), ("公牛插板", 25), ("监控电源", 15),
    ]:
        ...
        if existing:
            ...
            db_execute("INSERT INTO inventory_items (goods_id, quantity, location, min_stock, created_at) VALUES (?,?,?,?,?)",
    ...
    for name, price, desc in fees_data:
        ...
        if not existing:
            db_execute("INSERT INTO service_fees (name, unit_price, description, created_at) VALUES (?,?,?,?)",
